<script lang="ts">
	...
	let examAttempts = $derived(
		historyState.attempts.filter(
			(a): a is ExamAttempt => a.kind === 'exam' && a.certCode === data.certification.code
		)
	);
	let attemptsForThisCert = $derived(
		historyState.attempts.filter((a) => a.certCode === data.certification.code)
	);
...
{#if attemptsForThisCert.length > 0}
	<section class="stats">
		<div class="stat">
			<span class="stat-value">{attemptsForThisCert.length}</span>
